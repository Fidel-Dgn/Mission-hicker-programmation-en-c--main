 ...
 */
TM_SPI_DataSize_t BSP_TM_SPI_SetDataSize(SPI_TypeDef* SPIx, TM_SPI_DataSize_t DataSize) {
    TM_SPI_DataSize_t status = (SPIx->CR2 & SPI_CR2_DS_Msk) == SPI_DATASIZE_16BIT ? TM_SPI_DataSize_16b : TM_SPI_DataSize_8b;
	SPI_ID_e id = ((SPIx == SPI1)?SPI1_ID:(SPIx == SPI2)?SPI2_ID:SPI3_ID);
    ...
    if (DataSize == TM_SPI_DataSize_16b) {
        ...
        /* Set bits for 16-bit frame */
        SPIx->CR2 &= ~SPI_CR2_DS_Msk;
        SPIx->CR2 |= SPI_DATASIZE_16BIT;
    } else {
        hSPI[id].Init.DataSize = SPI_DATASIZE_8BIT;
        /* Set bits for 8-bit frame */
        SPIx->CR2 &= ~SPI_CR2_DS_Msk;
        SPIx->CR2 |= SPI_DATASIZE_8BIT;
    }
